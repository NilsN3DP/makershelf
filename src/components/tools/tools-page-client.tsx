  ...
  return (
    <div className="space-y-8">
      ...
      <section className="mobile-tools-hub panel p-4">
        ...
        <h2 className="mt-1 text-xl font-bold">Werkzeuge & Verwaltung</h2>
        <p className="mt-2 text-sm text-muted">
          Die wichtigsten Zusatzbereiche liegen mobil hier, damit die untere Navigation ruhig und schnell bedienbar bleibt.
        </p>
        <div className="mt-4 grid gap-3">
          <Link href="/indexing" className="mobile-tools-hub-card">
            <span>Indexierung</span>
            <small>Import-Ordner, Projektstruktur und Batch-Import</small>
          </Link>
          <Link href="/duplicates" className="mobile-tools-hub-card">
            <span>Duplikate</span>
            <small>Doppelte Dateien finden, prüfen und bereinigen</small>
          </Link>
          {settings.spoolmanEnabled && settings.spoolmanUrl.trim() && (
            <Link href="/spoolman" className="mobile-tools-hub-card">
              <span>Spoolman</span>
              <small>Filament- und Spulen-Verknüpfungen öffnen</small>
            </Link>
          )}
          {settings.filamentVaultEnabled && (
            <Link href="/filament" className="mobile-tools-hub-card">
              <span>Filament Vault</span>
              <small>Pro-Person-Bestand, Team Share, Orte und Verbrauch</small>
            </Link>
          )}
        </div>
      </section>
