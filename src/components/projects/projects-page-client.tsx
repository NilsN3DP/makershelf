...
export function ProjectsPageClient() {
  ...
  async function refreshGuestSubmissionLinks() {
    ...
    if (!response.ok) {
      throw new Error(payload.error || "Gast-Erstellungslinks konnten nicht geladen werden.");
    }
  ...
  useEffect(() => {
    void refreshGuestSubmissionLinks().catch((error) => {
      setGuestSubmissionStatus(error instanceof Error ? error.message : "Gast-Erstellungslinks konnten nicht geladen werden.");
    });
  }, []);

  async function handleCreateGuestSubmissionLink() {
    setIsGuestSubmissionBusy(true);
    setGuestSubmissionStatus("Erstelle Gast-Erstellungslink...");

    try {
      const response = await fetch("/api/guest-submission-links", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: "Gast-Projekt erstellen" }),
      });
      const payload = (await response.json()) as { link?: GuestSubmissionLink; error?: string };

      if (!response.ok || !payload.link) {
        throw new Error(payload.error || "Gast-Erstellungslink konnte nicht erstellt werden.");
      }
      ...
      try {
        ...
        setGuestSubmissionStatus("Gast-Erstellungslink erstellt. Kopieren im Browser nicht erlaubt, der Link steht unten.");
        return;
      }
      setGuestSubmissionStatus("Gast-Erstellungslink erstellt und kopiert.");
    } catch (error) {
      setGuestSubmissionStatus(error instanceof Error ? error.message : "Gast-Erstellungslink konnte nicht erstellt werden.");
    } finally {
  ...
  async function handleRevokeGuestSubmissionLink(linkId: string) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || "Gast-Erstellungslink konnte nicht deaktiviert werden.");
      }

      await refreshGuestSubmissionLinks();
      setGuestSubmissionStatus("Gast-Erstellungslink deaktiviert.");
    } catch (error) {
      setGuestSubmissionStatus(error instanceof Error ? error.message : "Gast-Erstellungslink konnte nicht deaktiviert werden.");
    } finally {
      setIsGuestSubmissionBusy(false);
    }
  }
                ...
                >
                  <div style={{ minWidth: 0, flex: "1 1 360px" }}>
                    <div className="responsive-actions">
                      ...
                      {isRevoked ? <span className="badge badge-danger">deaktiviert</span> : <span className="badge badge-success">aktiv</span>}
                    </div>
                  ...
                  <div className="responsive-actions">
                    <button
                      ...
                      onClick={() => {
                        ...
                        setGuestSubmissionStatus("Gast-Erstellungslink kopiert.");
                      }}
                    >
                      Kopieren
                    </button>
