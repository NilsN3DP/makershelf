...
export function ProjectDetailClient({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    async function loadGuestLinks() {
      try {
        ...
        if (!response.ok) {
          throw new Error(payload.error ?? "Gastlinks konnten nicht geladen werden.");
        }
        if (!ignore) {
  ...
  async function handleCreateGuestLink() {
    try {
      ...
      if (!response.ok || !payload.link) {
        throw new Error(payload.error ?? "Gastlink konnte nicht erstellt werden.");
      }
  ...
  async function handleRevokeGuestLink(linkId: string) {
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error ?? "Gastlink konnte nicht deaktiviert werden.");
      }
  ...
  async function openFilesViaBridge(files: PrintFile[]) {
    ...
    if (!response.ok || !data.manifestUrl) {
      throw new Error(data.error ?? "Bridge-Manifest konnte nicht erstellt werden.");
    }
      ...
      <section className="panel-technical grid gap-8 p-8 md:p-10 lg:grid-cols-[1.15fr_0.85fr]">
        <div className="relative space-y-6">
          <div className="flex flex-wrap gap-2">
            ...
            {project.favorite ? (
              <span className="badge-tech px-3 py-1 text-xs font-semibold">Favorit</span>
            ) : null}
          </div>
                  ...
                  <input
                    ...
                    onChange={async (event) => {
                      if (event.target.files?.length) {
                        ...
                        setUploadStatus(
                          `${result.added} Dateien importiert, ${result.skipped} übersprungen.`,
                        );
                        event.target.value = "";
                      }
                    }}
                  ...
                  <input
                    ...
                    onChange={async (event) => {
                      ...
                      if (file) {
                        ...
                        setUploadStatus(
                          `${result.added} Archiv-Dateien importiert, ${result.skipped} übersprungen.`,
                        );
                        event.target.value = "";
                      }
                    }}
                  ...
                  try {
                    ...
                    if (data.fileLinks?.length) {
                      ...
                      if (filesResponse.ok && remoteData.files?.length) {
                        ...
                        nextStatus = `${nextStatus} ${imported.added} Datei(en) aus dem Link importiert, ${imported.skipped + (remoteData.skipped ?? 0)} übersprungen.`;
                      } else if (!filesResponse.ok && remoteData.error) {
                        nextStatus = `${nextStatus} Dateiimport: ${remoteData.error}`;
                      }
                    }
