...
export function GuestProjectPage({ project, token }: GuestProjectPageProps) {
  ...
  async function handleProjectZipDownload(filesToDownload: PrintFile[], label: "all" | "selection") {
    if (!filesToDownload.length) {
      setExportStatus("Keine Dateien ausgewählt.");
      return;
    }

    setExportStatus(label === "all" ? "Erstelle Projekt-ZIP..." : "Erstelle ZIP aus Auswahl...");
    setExportProgress(0);
    try {
      const zip = new JSZip();
      for (const [index, file] of filesToDownload.entries()) {
        setExportProgress(Math.round((index / filesToDownload.length) * 45));
        setExportStatus(`Bereite Datei ${index + 1}/${filesToDownload.length} vor: ${file.name}`);
        const response = await fetch(guestDownloadUrl(token, file.id, true));
        if (!response.ok) throw new Error(`${file.name} konnte nicht geladen werden.`);
      ...
      setExportStatus("Packe ZIP-Archiv...");
      const blob = await zip.generateAsync(
        { type: "blob", compression: "DEFLATE", compressionOptions: { level: 6 } },
        (metadata) => {
          setExportProgress(Math.min(100, Math.round(45 + metadata.percent * 0.55)));
          setExportStatus(`Packe ZIP-Archiv... ${Math.round(metadata.percent)}%`);
        },
      );
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      const suffix = label === "all" ? "projekt" : "auswahl";
      link.download = `${project.title.replace(/[<>:"/\\|?*]+/g, "-") || "project"}-${suffix}.zip`;
      link.click();
      URL.revokeObjectURL(url);
      setExportStatus(label === "all" ? "Projekt-ZIP wurde erstellt." : "Auswahl-ZIP wurde erstellt.");
      setExportProgress(100);
      window.setTimeout(() => setExportProgress(null), 4000);
    } catch (error) {
      setExportProgress(null);
      setExportStatus(error instanceof Error ? error.message : "ZIP konnte nicht erstellt werden.");
    }
  }
        ...
        <section className="panel-technical grid gap-8 p-6 md:p-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            <div className="flex flex-wrap gap-2">
              <span className="badge-tech px-3 py-1 text-xs font-semibold">Gastzugriff</span>
            ...
            <div>
              ...
              <p className="mt-4 max-w-3xl text-muted">
                {project.description || "Dieses Projekt wurde per Gastlink freigegeben."}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
            ...
            <div className="grid gap-4 sm:grid-cols-4">
              ...
              <div className="metric-card">
                <p className="text-sm text-soft">Modelle</p>
              ...
              <div className="metric-card">
                <p className="text-sm text-soft">Aktualisiert</p>
          ...
          <div className="space-y-8">
            ...
            <section className="panel space-y-4 p-6">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  <p className="eyebrow">Projektstruktur</p>
                  <h2 className="mt-2 text-2xl font-bold text-main">Dateien im Projekt</h2>
                </div>
                <a
                  className="btn btn-secondary btn-sm"
                  href={guestProjectZipUrl(token)}
            ...
            <section className="panel space-y-4 p-6">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-bold text-main">Dokumente</h2>
                </div>
                {selectedPdf ? (
              ...
              {selectedPdf ? (
                ...
                <p className="rounded-xl border border-[var(--border)] bg-[var(--panel-muted)] p-4 text-sm text-muted">
                  Keine PDF-Dateien in diesem Projekt.
                </p>
              )}
          ...
          <aside className="space-y-8">
            ...
            <section className="panel space-y-4 p-6">
              ...
              <div className="grid gap-3 text-sm text-muted">
                <p><strong className="text-main">Quelle:</strong> {project.sourcePlatform || "-"}</p>
                <p><strong className="text-main">Original:</strong> {project.sourceUrl || "-"}</p>
                <p><strong className="text-main">Erstellt:</strong> {new Date(project.createdAt).toLocaleDateString("de-DE")}</p>
                <p><strong className="text-main">Dateien:</strong> {project.files.length}</p>
              </div>
            </section>
          </aside>
        </div>
