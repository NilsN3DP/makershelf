...
async function readFileAsDataUrl(file: File) {
  return new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error("Cover could not be read."));
...
async function extractArchiveWithServer(archiveFile: File) {
  ...
  if (!response.ok) {
    throw new Error(payload?.error || "Archive could not be extracted.");
  }
...
export function ProjectForm({
  ...
  const lockOptions: Array<{ key: ProjectLockField; label: string }> = [
    { key: "title", label: text(settings.language, "Titel sperren", "Lock title") },
    { key: "description", label: text(settings.language, "Beschreibung sperren", "Lock description") },
    { key: "author", label: text(settings.language, "Autor sperren", "Lock author") },
    { key: "license", label: text(settings.language, "Lizenz sperren", "Lock license") },
    { key: "tags", label: text(settings.language, "Tags sperren", "Lock tags") },
    { key: "coverImage", label: text(settings.language, "Cover sperren", "Lock cover") },
    { key: "sourceUrl", label: text(settings.language, "Original-Link sperren", "Lock original link") },
  ];
  ...
  async function handleCreateCategory() {
    if (!newCategoryName.trim()) {
      setCategoryError(text(settings.language, "Bitte gib einen Kategorienamen ein.", "Please enter a category name."));
      return undefined;
    }
    ...
    try {
      const createdId = await createCategory({
        ...
        description: newCategoryDescription.trim() || text(settings.language, "Benutzerdefinierte Kategorie", "Custom category"),
      ...
      setCategoryError(
        error instanceof Error ? error.message : text(settings.language, "Kategorie konnte nicht erstellt werden.", "Category could not be created."),
      );
      return undefined;
    } finally {
  ...
  async function ensureCategoryForImportedName(name?: string) {
    ...
    try {
      const createdId = await createCategory({
        name: trimmed,
        description: text(settings.language, "Automatisch aus dem Projektlink übernommen.", "Automatically imported from project link."),
        emoji: "🏷️",
      ...
      setCategoryError(
        error instanceof Error ? error.message : text(settings.language, "Kategorie konnte nicht automatisch angelegt werden.", "Category could not be created automatically."),
      );
      return "";
    }
  }
  ...
  async function loadWebsiteMetadata(url: string) {
    ...
    if (!response.ok) {
      throw new Error(data.error || text(settings.language, "Metadaten konnten nicht geladen werden.", "Metadata could not be loaded."));
    }
  ...
  function websiteMetadataStatus(data: WebsiteImportData) {
    ...
    const categoryNote = data.category
      ? text(settings.language, ` Kategorie: ${data.category}.`, ` Category: ${data.category}.`)
      : "";
    ...
    return data.fileLinks?.length
      ? text(
          ...
          `Metadata loaded from ${platform || "the website"}.${categoryNote} ${data.fileLinks.length} direct file links detected.`,
        )
      : data.warnings?.length
        ? text(
            settings.language,
            `Metadaten von ${platform || "der Website"} geladen.${categoryNote} ${data.warnings.join(" ")}`,
            `Metadata loaded from ${platform || "the website"}.${categoryNote} ${data.warnings.join(" ")}`,
          )
        : text(
            settings.language,
            ...
            `Metadata loaded from ${platform || "the website"}.${categoryNote} No direct file links detected, so only the info will be saved when creating the project.`,
          );
  }

  async function handleImportFromLink() {
    if (!sourceUrl.trim()) {
      setLinkImportStatus(text(settings.language, "Bitte zuerst einen Projektlink einfügen.", "Please enter a project link first."));
      return;
    }

    setLinkImportStatus(text(settings.language, "Link wird ausgewertet...", "Evaluating link..."));
    ...
    try {
      ...
      setLinkImportStatus(
        error instanceof Error ? error.message : text(settings.language, "Metadaten konnten nicht geladen werden.", "Metadata could not be loaded."),
      );
    }
  }
    ...
    if (!isZipArchiveFile(file.name)) {
      setInitialZipStatus(
        isSupportedArchiveFile(file.name)
          ? text(settings.language, "Archiv bereit: wird beim Speichern serverseitig entpackt.", "Archive ready: will be extracted server-side on save.")
          : text(settings.language, "Dieses Archivformat wird nicht unterstützt.", "This archive format is not supported."),
      );
      return;
    }

    setInitialZipStatus(text(settings.language, "ZIP wird analysiert...", "Analysing ZIP..."));

    try {
      const analysis = await analyzeZipForCreate(file);
      setInitialZipAnalysis(analysis);
      applyZipMetadataToForm(analysis.metadata);
      const baseStatus = text(
        settings.language,
        `ZIP bereit: ${analysis.files.length} Druckdatei(en), ${analysis.infoFiles.length} Info-Datei(en), ${analysis.imageCount} Bild(er) erkannt.`,
        `ZIP ready: ${analysis.files.length} print file(s), ${analysis.infoFiles.length} info file(s), ${analysis.imageCount} image(s) detected.`,
      );

      if (analysis.sourceUrl) {
        setInitialZipStatus(`${baseStatus} ${text(settings.language, "Lade genaue Projektinfos vom Link...", "Loading detailed project info from link...")}`);
        try {
          ...
          setInitialZipStatus(
            metadataError instanceof Error
              ? `${baseStatus} ${text(settings.language, `Link erkannt (${analysis.sourceUrl}), Website-Daten konnten aber nicht geladen werden: ${metadataError.message}`, `Link detected (${analysis.sourceUrl}), website data could not be loaded: ${metadataError.message}`)}`
              : `${baseStatus} ${text(settings.language, `Link erkannt (${analysis.sourceUrl}), Website-Daten konnten aber nicht geladen werden.`, `Link detected (${analysis.sourceUrl}), website data could not be loaded.`)}`,
          );
        }
        return;
      }
      ...
      setInitialZipStatus(
        error instanceof Error
          ? text(settings.language, `ZIP konnte nicht analysiert werden: ${error.message}`, `ZIP could not be analysed: ${error.message}`)
          : text(settings.language, "ZIP konnte nicht analysiert werden.", "ZIP could not be analysed."),
      );
    }
  }
    ...
    setInitialPrintFileStatus(
      supportedFiles.length
        ? text(
            ...
            `${supportedFiles.length} project file(s) queued for the new project${skipped ? `, ${skipped} file(s) skipped` : ""}. Archives will be extracted on save.`,
          )
        : skipped
          ? text(settings.language, "Keine unterstützten Projektdateien ausgewählt.", "No supported project files selected.")
          : "",
    );
  }
    ...
    <div>
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">
            {mode === "create" ? text(settings.language, "Neues Projekt", "New project") : (project?.title ?? text(settings.language, "Projekt bearbeiten", "Edit project"))}
          </h1>
          <p className="page-subtitle">
            {mode === "create" ? text(settings.language, "Projekt anlegen und Dateien hinzufügen", "Create project and add files") : text(settings.language, "Projektdaten aktualisieren", "Update project data")}
          </p>
        </div>
      ...
      <form
        onSubmit={async (event) => {
          ...
          if (!title.trim()) {
            nextFieldErrors.title = text(settings.language, "Bitte einen Projekttitel angeben.", "Please enter a project title.");
          }

          if (!description.trim()) {
            nextFieldErrors.description = text(settings.language, "Bitte eine kurze Projektbeschreibung angeben.", "Please enter a short project description.");
          }

          if (Object.keys(nextFieldErrors).length > 0) {
            setFieldErrors(nextFieldErrors);
            setFormError(text(settings.language, "Bitte prüfe die markierten Pflichtfelder, bevor du das Projekt speicherst.", "Please check the highlighted required fields before saving the project."));
            return;
          }
          ...
          try {
            ...
            if (!resolvedCategoryId) {
              setCategoryError(text(settings.language, "Bitte wähle eine Kategorie oder lege eine neue Kategorie mit Namen an.", "Please select a category or create a new one with a name."));
              setSaving(false);
              return;
            }
            ...
            if (mode === "create") {
              ...
              if (initialZipFile) {
                if (initialZipMode === "archive") {
                  setInitialZipStatus(text(settings.language, "Projekt wird erstellt und Archiv wird gespeichert...", "Creating project and saving archive..."));
                  ...
                  setInitialZipStatus(text(settings.language, "Archiv wird entpackt und Projekt wird erstellt...", "Extracting archive and creating project..."));
                  ...
                  if (archiveExtracted.files.length === 0) {
                    throw new Error(
                      text(settings.language, "Das Archiv enthält keine unterstützten Druckdateien. Nutze ein Archiv mit STL, OBJ, 3MF, STEP, GCODE, AMF, PLY oder PDF.", "The archive contains no supported print files. Use an archive with STL, OBJ, 3MF, STEP, GCODE, AMF, PLY or PDF."),
                    );
                  }
                ...
                setInitialZipStatus(
                  zipSkipped > 0
                    ? text(settings.language, `Archiv übernommen, ${zipSkipped} Datei(en) übersprungen.`, `Archive imported, ${zipSkipped} file(s) skipped.`)
                    : text(settings.language, "Archiv vollständig übernommen.", "Archive fully imported."),
                );
              } else if (directFiles.length > 0) {
                setInitialPrintFileStatus(text(settings.language, "Projekt wird mit Projektdateien erstellt. Archive werden entpackt...", "Creating project with print files. Archives will be extracted..."));
                  const result = await createProjectWithFiles(
                    projectPayload,
                    directFiles,
                    "upload",
                ...
                setInitialPrintFileStatus(
                  result.skipped > 0
                    ? text(settings.language, `${result.added} Projektdatei(en) übernommen, ${result.skipped} übersprungen.`, `${result.added} project file(s) imported, ${result.skipped} skipped.`)
                    : text(settings.language, `${result.added} Projektdatei(en) übernommen.`, `${result.added} project file(s) imported.`),
                );
              } else {
              ...
              if (sourceUrlForMetadata && metadataSourceUrl) {
                try {
                  setLinkImportStatus(text(settings.language, "PDF-Metadaten erkannt. Lade Projektinfos und Cover...", "PDF metadata detected. Loading project info and cover..."));
                  await importMetadataForProject(nextId, sourceUrlForMetadata);
                  setLinkImportStatus(text(settings.language, "PDF-Metadaten übernommen.", "PDF metadata imported."));
                } catch (error) {
                  setLinkImportStatus(
                    error instanceof Error
                      ? text(settings.language, `PDF-Link erkannt, Metadaten konnten aber nicht übernommen werden: ${error.message}`, `PDF link detected, metadata could not be imported: ${error.message}`)
                      : text(settings.language, "PDF-Link erkannt, Metadaten konnten aber nicht übernommen werden.", "PDF link detected, metadata could not be imported."),
                  );
                }
              }

              if (importedFileLinks.length > 0) {
                setLinkImportStatus(text(settings.language, "Projekt erstellt. Lade verknüpfte Dateien herunter...", "Project created. Downloading linked files..."));
                ...
                if (response.ok && remoteData.files?.length) {
                  ...
                  setLinkImportStatus(
                    text(settings.language, `${result.added} Dateien aus dem Link importiert, ${result.skipped + (remoteData.skipped ?? 0)} übersprungen.`, `${result.added} files imported from link, ${result.skipped + (remoteData.skipped ?? 0)} skipped.`),
                  );
                } else if (response.ok) {
                  setLinkImportStatus(
                    text(settings.language, "Projekt erstellt. Es konnten keine direkt herunterladbaren Projektdateien aus dem Link übernommen werden.", "Project created. No directly downloadable project files could be imported from the link."),
                  );
            ...
            setFormError(
              error instanceof Error ? error.message : text(settings.language, "Projekt konnte nicht gespeichert werden.", "Project could not be saved."),
            );
          } finally {
            setSaving(false);
          }
      ...
      >
        ...
        {mode === "create" && (
          <div className="panel" style={{ marginBottom: "1.25rem", borderLeft: "3px solid var(--primary)" }}>
            <div className="panel-section" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: "16px" }}>
              <div>
                <div style={{ fontSize: "11px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--primary)", marginBottom: "4px" }}>{text(settings.language, "Datei-Import", "File import")}</div>
                <h2 style={{ fontSize: "16px", fontWeight: 700, color: "var(--text-main)", margin: 0 }}>{text(settings.language, "Archiv oder Druckdateien auswählen", "Select archive or print files")}</h2>
                <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "4px", lineHeight: 1.5 }}>
                  {text(
                    settings.language,
                    "ZIP-Archive werden sofort analysiert, RAR/7z/TAR/GZ beim Speichern serverseitig entpackt. Alternativ einzelne STL, 3MF, STEP oder GCODE-Dateien anhängen.",
                    "ZIP archives are analysed immediately; RAR/7z/TAR/GZ are extracted server-side on save. Alternatively, attach individual STL, 3MF, STEP or GCODE files.",
                  )}
                </p>
              </div>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                <label className="btn btn-primary" style={{ cursor: "pointer" }}>
                  {text(settings.language, "Archiv auswählen", "Select archive")}
                ...
                <label className="btn btn-secondary" style={{ cursor: "pointer" }}>
                  {text(settings.language, "3D-Dateien", "3D files")}
                ...
                <label className="btn btn-ghost" style={{ cursor: "pointer" }}>
                  {text(settings.language, "Ordner", "Folder")}
            ...
            <div className="panel-section" style={{ display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: "12px", alignItems: "start" }}>
              <div style={{ background: "var(--panel-muted)", borderRadius: "8px", padding: "10px 14px" }}>
                <div style={{ fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-soft)", marginBottom: "4px" }}>{text(settings.language, "Archiv", "Archive")}</div>
                <div style={{ fontSize: "13.5px", fontWeight: 600, color: "var(--text-main)" }}>
                  {initialZipFile ? initialZipFile.name : text(settings.language, "Nicht ausgewählt", "Not selected")}
                </div>
                {initialZipFile && (
                  <div style={{ fontSize: "12px", color: "var(--text-muted)", marginTop: "2px" }}>
                    {(initialZipFile.size / 1024 / 1024).toFixed(1)} MB · {initialZipMode === "extract" ? text(settings.language, "wird entpackt", "will be extracted") : text(settings.language, "als Archiv", "as archive")}
                  </div>
                )}
              </div>
              <div style={{ background: "var(--panel-muted)", borderRadius: "8px", padding: "10px 14px" }}>
                <div style={{ fontSize: "11px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-soft)", marginBottom: "4px" }}>{text(settings.language, "Druckdateien", "Print files")}</div>
                <div style={{ fontSize: "13.5px", fontWeight: 600, color: "var(--text-main)" }}>
                  {initialPrintFiles.length ? text(settings.language, `${initialPrintFiles.length} Datei(en)`, `${initialPrintFiles.length} file(s)`) : text(settings.language, "Nicht ausgewählt", "Not selected")}
                </div>
              ...
              <select className="input select" value={initialZipMode} onChange={(event) => setInitialZipMode(event.target.value as "extract" | "archive")} style={{ minWidth: "170px" }}>
                <option value="extract">{text(settings.language, "Entpackt importieren", "Import extracted")}</option>
                <option value="archive">{text(settings.language, "Als Archiv speichern", "Store as archive")}</option>
              </select>
            </div>
            ...
            <div className="panel-section" style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
              <label style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "13px", fontWeight: 500, cursor: "pointer", color: "var(--text-main)" }}>
                ...
                {text(settings.language, "Ordnerstruktur übernehmen", "Preserve folder structure")}
              </label>
              {initialZipFile && (
                <button type="button" onClick={() => { setInitialZipFile(null); setInitialZipAnalysis(null); setInitialZipStatus(""); }} className="btn btn-ghost btn-sm">
                  {text(settings.language, "Archiv entfernen", "Remove archive")}
                </button>
              )}
              {initialPrintFiles.length > 0 && (
                <button type="button" onClick={() => { setInitialPrintFiles([]); setInitialPrintFileStatus(""); }} className="btn btn-ghost btn-sm">
                  {text(settings.language, "Dateien entfernen", "Remove files")}
                </button>
              )}
          ...
          <div className="panel-section" style={{ borderBottom: "1px solid var(--border)" }}>
            <h2 style={{ fontSize: "15px", fontWeight: 700, margin: 0, color: "var(--text-main)" }}>{text(settings.language, "Projektinfos", "Project info")}</h2>
            <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "2px" }}>{text(settings.language, "Titel, Beschreibung, Kategorie und Lizenz", "Title, description, category and licence")}</p>
          </div>
          <div className="panel-section" style={{ display: "grid", gap: "16px" }}>
            <div>
              <label className="label">{text(settings.language, "Titel *", "Title *")}</label>
              <input
                className="input"
                value={title}
                onChange={(event) => { setTitle(event.target.value); setFieldErrors((current) => ({ ...current, title: undefined })); }}
                placeholder={text(settings.language, "z. B. Open Frame Filament Rack", "e.g. Open Frame Filament Rack")}
            ...
            <div>
              <label className="label">{text(settings.language, "Beschreibung *", "Description *")}</label>
              <textarea
                className="input"
                value={description}
                onChange={(event) => { setDescription(event.target.value); setFieldErrors((current) => ({ ...current, description: undefined })); }}
                placeholder={text(settings.language, "Projektbeschreibung, Druckhinweise und Besonderheiten...", "Project description, print notes and special details...")}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                ...
                >
                  ...
                  <option value={NEW_CATEGORY_VALUE}>{text(settings.language, "+ Neue Kategorie anlegen", "+ Create new category")}</option>
                </select>
              </div>
              <div>
                <label className="label">{text(settings.language, "Lizenz", "Licence")}</label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                <label className="label">{text(settings.language, "Modell-Designer (Autor)", "Model designer (author)")}</label>
                <input className="input" value={author} onChange={(event) => setAuthor(event.target.value)} placeholder={text(settings.language, "z. B. Printables-Username", "e.g. Printables username")} />
              </div>
              <div>
                <label className="label">{text(settings.language, "Tags", "Tags")}</label>
                <input className="input" value={tags} onChange={(event) => setTags(event.target.value)} placeholder={text(settings.language, "mechanisch, organizer, remix", "mechanical, organizer, remix")} />
              </div>
            </div>
          </div>
        </div>
        ...
        {!categoryId && (
          <div className="panel" style={{ marginBottom: "1.25rem", borderLeft: "3px solid var(--info)" }}>
            <div className="panel-section">
              <h3 style={{ fontSize: "14px", fontWeight: 700, margin: "0 0 6px", color: "var(--text-main)" }}>{text(settings.language, "Neue Kategorie anlegen", "Create new category")}</h3>
              <p style={{ fontSize: "12.5px", color: "var(--text-muted)", margin: "0 0 12px" }}>
                {text(settings.language, "Vergib einen Namen, sonst kann das Projekt keiner Kategorie zugeordnet werden.", "Enter a name, otherwise the project cannot be assigned to a category.")}
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
                <input className="input" value={newCategoryName} onChange={(event) => { setNewCategoryName(event.target.value); setCategoryError(""); }} placeholder={text(settings.language, "Kategoriename", "Category name")} />
                <input className="input" value={newCategoryDescription} onChange={(event) => setNewCategoryDescription(event.target.value)} placeholder={text(settings.language, "Beschreibung", "Description")} />
              </div>
              <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
                ...
                <button type="button" onClick={() => void handleCreateCategory()} className="btn btn-secondary" disabled={creatingCategory}>
                  {creatingCategory ? text(settings.language, "Wird erstellt...", "Creating...") : text(settings.language, "Kategorie erstellen", "Create category")}
                </button>
              </div>
        ...
        <div className="panel" style={{ marginBottom: "1.25rem" }}>
          <div className="panel-section" style={{ borderBottom: "1px solid var(--border)" }}>
            <h2 style={{ fontSize: "15px", fontWeight: 700, margin: 0, color: "var(--text-main)" }}>{text(settings.language, "Quelle & Zuordnung", "Source & assignment")}</h2>
            <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "2px" }}>{text(settings.language, "Creator, Quell-URL, Cover und Status", "Creator, source URL, cover and status")}</p>
          </div>
          <div className="panel-section" style={{ display: "grid", gap: "16px" }}>
            {/* URL import row */}
            <div>
              <label className="label">{text(settings.language, "Quell-URL", "Source URL")}</label>
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <button type="button" onClick={() => void handleImportFromLink()} className="btn btn-secondary" style={{ whiteSpace: "nowrap" }}>
                  {text(settings.language, "Daten laden", "Load data")}
                </button>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                <label className="label">{text(settings.language, "Creator", "Creator")}</label>
              ...
              <div>
                <label className="label">{text(settings.language, "Unterordner", "Subfolder")}</label>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 160px", gap: "12px", alignItems: "start" }}>
              <div>
                <label className="label">{text(settings.language, "Cover-Bild URL", "Cover image URL")}</label>
                ...
                <label className="btn btn-ghost btn-sm" style={{ cursor: "pointer", display: "inline-flex" }}>
                  {text(settings.language, "Bild hochladen", "Upload image")}
              ...
              <div style={{ background: "var(--panel-muted)", borderRadius: "8px", overflow: "hidden", aspectRatio: "16/9", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {coverImage ? (
                  ...
                  <span style={{ fontSize: "12px", color: "var(--text-soft)", textAlign: "center", padding: "8px" }}>{text(settings.language, "Kein Cover", "No cover")}</span>
                )}
              </div>
            </div>
            ...
            <label className="toggle">
              ...
              <span style={{ fontSize: "13.5px", fontWeight: 500, color: "var(--text-main)" }}>{text(settings.language, "Projekt aktiv (in Bearbeitung)", "Project active (in progress)")}</span>
            </label>
          </div>
        </div>
        ...
        <div className="panel" style={{ marginBottom: "1.25rem" }}>
          <div className="panel-section" style={{ borderBottom: "1px solid var(--border)" }}>
            <h2 style={{ fontSize: "15px", fontWeight: 700, margin: 0, color: "var(--text-main)" }}>{text(settings.language, "Organisation", "Organisation")}</h2>
            <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "2px" }}>{text(settings.language, "BOM, Projektschritte und Links (optional)", "BOM, project steps and links (optional)")}</p>
          </div>
          <div className="panel-section" style={{ display: "grid", gap: "20px" }}>
            {/* BOM */}
            <div>
              <label className="label">{text(settings.language, "BOM / Einkaufsliste", "BOM / shopping list")}</label>
              <textarea className="input" value={shoppingList} onChange={(event) => setShoppingList(event.target.value)} placeholder={text(settings.language, "Je Eintrag eine Zeile", "One entry per line")} style={{ minHeight: "80px", resize: "vertical" }} />
            </div>

            {/* Steps */}
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <span className="label" style={{ margin: 0 }}>{text(settings.language, "Projektschritte", "Project steps")}</span>
                <button type="button" onClick={() => setSteps((current) => [...current, ""])} className="btn btn-ghost btn-sm">{text(settings.language, "+ Schritt", "+ Step")}</button>
              </div>
              <div style={{ display: "grid", gap: "6px" }}>
                {steps.map((step, index) => (
                  <div key={`step-${index}`} style={{ display: "flex", gap: "6px" }}>
                    <input className="input" value={step} onChange={(event) => setSteps((current) => current.map((item, i) => i === index ? event.target.value : item))} placeholder={text(settings.language, `Schritt ${index + 1}`, `Step ${index + 1}`)} style={{ flex: 1 }} />
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <span className="label" style={{ margin: 0 }}>{text(settings.language, "Links", "Links")}</span>
              ...
              <div style={{ display: "grid", gap: "6px" }}>
                {links.map((link, index) => (
                  <div key={link.id} style={{ display: "grid", gridTemplateColumns: "0.7fr 1fr auto", gap: "6px" }}>
                    <input className="input" value={link.label} onChange={(event) => setLinks((current) => current.map((item, i) => i === index ? { ...item, label: event.target.value } : item))} placeholder={text(settings.language, "Titel", "Title")} />
        ...
        {mode === "edit" && (
          <div className="panel" style={{ marginBottom: "1.25rem" }}>
            <div className="panel-section" style={{ borderBottom: "1px solid var(--border)" }}>
              <h2 style={{ fontSize: "15px", fontWeight: 700, margin: 0, color: "var(--text-main)" }}>{text(settings.language, "Import-Schutz", "Import protection")}</h2>
              <p style={{ fontSize: "13px", color: "var(--text-muted)", marginTop: "2px" }}>{text(settings.language, "Gesperrte Felder werden durch Website-Importe nicht überschrieben", "Locked fields will not be overwritten by website imports")}</p>
            </div>
        ...
        <div style={{ position: "sticky", bottom: "1rem", zIndex: 10, background: "var(--panel)", border: "1px solid var(--border)", borderRadius: "12px", padding: "14px 16px", boxShadow: "var(--shadow-lg)", display: "flex", gap: "10px", flexWrap: "wrap", alignItems: "center" }}>
          <button className="btn btn-primary btn-lg" disabled={saving}>
            {saving
              ? (mode === "create" ? text(settings.language, "Wird angelegt...", "Creating...") : text(settings.language, "Speichert...", "Saving..."))
              : (mode === "create" ? text(settings.language, "Projekt anlegen", "Create project") : text(settings.language, "Änderungen speichern", "Save changes"))}
          </button>
          ...
          {mode === "edit" && (
            ...
            >
              {text(settings.language, "Projekt löschen", "Delete project")}
            </button>
          )}
        </div>
      </form>
