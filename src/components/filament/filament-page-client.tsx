...
function statusLabel(status: FilamentStatus) {
  if (status === "FULL") return "Eingeschweißt";
...
function MaterialCombobox({ value, suggestions, onInputChange, onSelect }: MaterialComboboxProps) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <input
        ...
        placeholder="Material wählen oder frei eingeben"
        required
      />
...
export function FilamentPageClient({ initialSpoolId }: FilamentPageClientProps) {
  ...
  async function startBarcodeCamera() {
    ...
    try {
      stopBarcodeCamera();
      setBarcodeReaderMessage("Kamera wird geöffnet...");
      ...
      setBarcodeReaderMessage(
        error instanceof Error
          ? `Kamera konnte nicht geöffnet werden: ${error.message}`
    ...
    try {
      ...
      toast(
        payload.parser === "prusament-live"
          ? "Prusament-Daten übernommen."
          : "Scan-Daten ins Formular übernommen.",
        "success",
      );
    } catch (error) {
  ...
  async function deleteSpool(id: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(payload.error ?? "Spule konnte nicht gelöscht werden.");
      }
      setSpools((c) => c.filter((s) => s.id !== id));
      setDeleteConfirmId(null);
      if (drawerSpool?.id === id) closeDrawer();
      toast("Spule gelöscht.", "success");
    } catch (error) {
      toast(error instanceof Error ? error.message : "Fehler.", "error");
    } finally {
      setDeleting(false);
  ...
  async function copyTagPayload(spool: FilamentSpool) {
    ...
    try {
      ...
      toast("OpenPrintTag-Payload kopiert.", "success");
    } catch {
  ...
  function applyOpenPrintTagPayload(payload: Record<string, unknown>) {
    ...
    if (existing) {
      openEdit(existing);
      toast("NFC-Tag erkannt — bestehende Spule geöffnet.", "success");
      return;
    }
    ...
    toast("OpenPrintTag eingelesen — Daten ins Formular übernommen.", "success");
  }
  ...
  const startNfcRead = useCallback(async () => {
    ...
    try {
      ...
      if (result.type === "url") {
        ...
        toast("NFC-URL erkannt — Daten übernommen.", "success");
        return;
      }

      toast("NFC-Tag gelesen, aber kein OpenPrintTag-Format erkannt.", "info");
    } catch (error) {
      if (error instanceof DOMException && error.name === "AbortError") return;
      toast(error instanceof Error ? error.message : "NFC-Fehler.", "error");
    } finally {
      setNfcMode(null);
      nfcAbortRef.current = null;
    }
  ...
  const startNfcWrite = useCallback(
    async (spool: FilamentSpool) => {
      ...
      try {
        ...
        toast("NFC-Tag erfolgreich beschrieben.", "success");
      } catch (error) {
        if (error instanceof DOMException && error.name === "AbortError") return;
        toast(error instanceof Error ? error.message : "NFC-Schreiben fehlgeschlagen.", "error");
      } finally {
        setNfcMode(null);
        nfcAbortRef.current = null;
      }
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle" style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap" }}>
            <span>{stats.total} Spulen · {stats.totalKg} kg Bestand</span>
            {stats.lowOrEmpty > 0 && (
              <span className="badge badge-warning">{stats.lowOrEmpty} niedrig/leer</span>
            )}
          </p>
        </div>
      ...
      {filteredSpools.length === 0 ? (
        <div className="panel">
          <div className="empty-state">
            ...
            <p className="empty-state-title">Keine Spulen</p>
            <p className="empty-state-desc">
              {search || scopeFilter !== "ALL"
                ? "Keine Spulen gefunden. Filter oder Suche anpassen."
                : "Noch keine Spulen vorhanden. Klicke \"+ Neue Spule\" um loszulegen."}
            </p>
          </div>
        </div>
      ) : (
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "4px", alignItems: "center", flexShrink: 0 }}>
                    <button
                      type="button"
                      className="btn btn-sm btn-secondary"
                      title={nfcAvailable ? "OpenPrintTag auf NFC schreiben" : "NFC schreiben, falls der Browser den Leser unterstützt"}
                    ...
                    <button
                      ...
                      onClick={() => setExpandedSpoolId((id) => (id === spool.id ? null : spool.id))}
                      title={isExpanded ? "Zuklappen" : "Details"}
                    >
                  ...
                  >
                    ...
                    {(spool.nozzleTempMinC || spool.bedTempC || spool.dryingTempC || spool.flowFactor || spool.densityGcm3) && (
                      <div style={{ display: "grid", gap: "3px" }}>
                        <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text-soft)", margin: "0 0 4px" }}>Druck</p>
                        ...
                        {spool.bedTempC && <p style={{ color: "var(--text-muted)", margin: 0 }}>Bett {spool.bedTempC}°C</p>}
                        {spool.dryingTempC && <p style={{ color: "var(--text-muted)", margin: 0 }}>Trocknen {spool.dryingTempC}°C · {spool.dryingHours ?? "?"}h</p>}
                    ...
                    {(spool.netWeightGrams != null || spool.remainingWeightGrams != null) && (
                      <div style={{ display: "grid", gap: "3px" }}>
                        <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text-soft)", margin: "0 0 4px" }}>Gewicht</p>
                        {spool.netWeightGrams != null && <p style={{ color: "var(--text-muted)", margin: 0 }}>Netto {spool.netWeightGrams} g</p>}
                        {spool.tareWeightGrams != null && <p style={{ color: "var(--text-soft)", margin: 0 }}>Spule (leer) {spool.tareWeightGrams} g</p>}
                        {spool.remainingWeightGrams != null && <p style={{ color: "var(--text-main)", fontWeight: 600, margin: 0 }}>Rest {spool.remainingWeightGrams} g</p>}
                      </div>
                    )}
                    ...
                    {(spool.vendorName || spool.purchaseDate || spool.purchasePriceCents != null || spool.lotNumber) && (
                      <div style={{ display: "grid", gap: "3px" }}>
                        <p style={{ fontSize: "10px", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--text-soft)", margin: "0 0 4px" }}>Einkauf</p>
                        {spool.vendorName && <p style={{ color: "var(--text-main)", margin: 0 }}>{spool.vendorName}</p>}
                        {spool.purchaseDate && <p style={{ color: "var(--text-muted)", margin: 0 }}>{new Date(spool.purchaseDate).toLocaleDateString("de-DE", { day: "2-digit", month: "2-digit", year: "numeric" })}</p>}
          ...
          >
            ...
            >
              <h2 style={{ fontSize: "15px", fontWeight: 700, margin: 0 }}>
                {drawerMode === "add"
                  ...
                  : drawerMode === "batch"
                    ? "Batch-Import"
                    : `${drawerSpool?.name ?? "Spule"} bearbeiten`}
              </h2>
            ...
            >
              ...
              {drawerMode === "batch" && (
                ...
                >
                  <SectionLabel>Anzahl Spulen</SectionLabel>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    ...
                    <p style={{ fontSize: "12.5px", color: "var(--text-muted)", margin: 0 }}>
                      {batchQty > 1
                        ? `Erstellt ${batchQty} identische Spulen — Name wird automatisch durchnummeriert (#1, #2 …)`
                        : "Erstellt eine einzelne Spule."}
                    </p>
                  </div>
                </div>
              )}
              ...
              {(drawerMode === "add" || drawerMode === "batch") && (
                <div style={{ display: "grid", gap: "8px" }}>
                  <SectionLabel>Verpackung / Tag einlesen</SectionLabel>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr auto auto auto", gap: "6px" }}>
                    <input
                      ...
                      placeholder="Verpackungsbarcode, SKU, QR-Link oder OpenPrintTag scannen"
                    />
                    <button
                      type="button"
                    ...
                    <button
                      type="button"
                      className="btn btn-secondary btn-sm"
                      title="Barcode per Kamera, USB-Scanner oder Tastatureingabe lesen"
              ...
              <div style={{ display: "grid", gap: "10px" }}>
                ...
                <Field label="Sichtbarkeit">
                  ...
                  >
                    ...
                    <option value="TEAM">Geteilt</option>
                  </select>
                </Field>
                <Field label="Name *">
                  <input
                    ...
                    placeholder="z.B. PLA Matt Schwarz"
                    required
                  />
                </Field>
                <FieldGrid cols={2}>
                ...
                <button className="btn btn-primary" type="submit" disabled={saving}>
                  {saving
                    ...
                    : drawerMode === "add"
                      ? "Spule anlegen"
                      : drawerMode === "batch"
                        ? `${batchQty} Spule${batchQty !== 1 ? "n" : ""} anlegen`
                        : "Speichern"}
                </button>
