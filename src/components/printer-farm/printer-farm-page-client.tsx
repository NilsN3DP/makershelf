...
type MaintenancePlan = {
  ...
  notes: string;
  active: boolean;
  daysUntilDue?: number | null; // set by workspace-wide endpoint
};
...
function PrinterDrawer({ lang, printer, groups, onClose, onSaved }: {
  ...
  async function save() {
    if (!name.trim() || (!isDummy && !apiUrl.trim())) return;
    setSaving(true);
    setError("");
  ...
  return (
    ...
    >
      <div style={{ width: "min(480px, 100vw)", height: "100dvh", background: "var(--panel)", borderLeft: "1px solid var(--border)", overflowY: "auto", padding: "24px", display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        {/* Dummy printer toggle */}
        <label style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 12px", background: "var(--panel-muted)", borderRadius: "8px", cursor: "pointer" }}>
          ...
          <div>
            <p style={{ fontSize: "13px", fontWeight: 600, color: "var(--text-main)", margin: 0 }}>
              {text(lang, "Dummy / Offline-Drucker", "Dummy / Offline Printer")}
            </p>
        ...
        <div style={{ display: "flex", gap: "8px", marginTop: "auto", paddingTop: "8px" }}>
          ...
          <button type="button" className="btn btn-primary" style={{ flex: 2 }} disabled={!name.trim() || (!isDummy && !apiUrl.trim()) || saving} onClick={() => void save()}>
...
function PlanModal({ lang, plan, printerId, onClose, onSaved }: {
  lang: Language;
  ...
  const [planType, setPlanType] = useState(plan?.type ?? "cleaning");
  const [label, setLabel] = useState(plan?.label ?? "");
  const [intervalDays, setIntervalDays] = useState(String(plan?.intervalDays ?? 30));
  const [notifyDaysBefore, setNotifyDaysBefore] = useState(String(plan?.notifyDaysBefore ?? 3));
  const [lastPerformedAt, setLastPerformedAt] = useState(
    plan?.lastPerformedAt ? new Date(plan.lastPerformedAt).toISOString().slice(0, 16) : ""
  );
  const [planNotes, setPlanNotes] = useState(plan?.notes ?? "");
  const [saving, setSaving] = useState(false);

  async function save() {
    if (!intervalDays || Number(intervalDays) < 1) return;
    setSaving(true);
    const body = {
      type: planType,
      label: label.trim(),
      ...
      lastPerformedAt: lastPerformedAt ? new Date(lastPerformedAt).toISOString() : null,
      notes: planNotes.trim(),
    };
    const url = plan
      ? `/api/printer-farm/printers/${printerId}/maintenance-plans/${plan.id}`
      : `/api/printer-farm/printers/${printerId}/maintenance-plans`;
  ...
  return (
    <div
      ...
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: "14px", padding: "24px", width: "min(440px, 95vw)", display: "flex", flexDirection: "column", gap: "14px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <h3 style={{ fontWeight: 700, fontSize: "15px", margin: 0 }}>
            {plan ? text(lang, "Plan bearbeiten", "Edit Plan") : text(lang, "Plan hinzufügen", "Add Plan")}
          </h3>
          <button type="button" className="btn btn-ghost btn-sm" onClick={onClose}><IconX /></button>
        </div>
        ...
        <div>
          <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "var(--text-muted)", marginBottom: "5px" }}>{text(lang, "Eigene Bezeichnung (optional)", "Custom label (optional)")}</label>
          <input className="input" style={{ width: "100%" }} value={label} onChange={(e) => setLabel(e.target.value)} placeholder={maintenanceLabel(planType, lang)} />
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          <div>
            <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "var(--text-muted)", marginBottom: "5px" }}>{text(lang, "Intervall (Tage)", "Interval (days)")}</label>
            <input className="input" style={{ width: "100%" }} type="number" min="1" value={intervalDays} onChange={(e) => setIntervalDays(e.target.value)} />
          </div>
          <div>
            <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "var(--text-muted)", marginBottom: "5px" }}>{text(lang, "Erinnerung (Tage vorher)", "Notify (days before)")}</label>
            <input className="input" style={{ width: "100%" }} type="number" min="0" value={notifyDaysBefore} onChange={(e) => setNotifyDaysBefore(e.target.value)} />
          </div>
        </div>

        <div>
          <label style={{ display: "block", fontSize: "12px", fontWeight: 600, color: "var(--text-muted)", marginBottom: "5px" }}>{text(lang, "Zuletzt durchgeführt (optional)", "Last performed (optional)")}</label>
          <input className="input" style={{ width: "100%" }} type="datetime-local" value={lastPerformedAt} onChange={(e) => setLastPerformedAt(e.target.value)} />
        </div>
        ...
        <div>
          ...
          <textarea className="input" style={{ width: "100%", resize: "vertical" }} rows={2} value={planNotes} onChange={(e) => setPlanNotes(e.target.value)} />
        </div>

        <div style={{ display: "flex", gap: "8px" }}>
          <button type="button" className="btn btn-ghost" style={{ flex: 1 }} onClick={onClose}>{text(lang, "Abbrechen", "Cancel")}</button>
          <button type="button" className="btn btn-primary" style={{ flex: 2 }} disabled={!intervalDays || Number(intervalDays) < 1 || saving} onClick={() => void save()}>
            {saving ? "…" : text(lang, "Speichern", "Save")}
          </button>
        </div>
      </div>
...
function PrinterDetailPanel({ lang, printer, status, onClose, onEdit, onStatusRefresh, onPlansChanged, workspacePlans }: {
  ...
  const fetchMaintenancePlans = useCallback(async () => {
    const res = await fetch(`/api/printer-farm/printers/${printer.id}/maintenance-plans`);
    if (res.ok) {
      const data = await res.json() as { plans: MaintenancePlan[] };
  ...
  useEffect(() => {
    const timeout = window.setTimeout(() => {
      if (activeTab === "maintenance") { void fetchMaintenance(); void fetchMaintenancePlans(); }
      if (activeTab === "filament") { void fetchFilamentLogs(); void fetchSpools(); }
    }, 0);
    return () => window.clearTimeout(timeout);
  }, [activeTab, fetchMaintenance, fetchMaintenancePlans, fetchFilamentLogs, fetchSpools]);

  function selectSpool(nextSpoolId: string) {
    setSpoolId(nextSpoolId);
    const spool = spools.find((sp) => sp.id === nextSpoolId);
    if (spool) {
      setMatName(spool.material ?? "");
      setColName(spool.colorName ?? "");
    }
  }
  ...
  async function markPlanDone(plan: MaintenancePlan) {
    await fetch(`/api/printer-farm/printers/${printer.id}/maintenance`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ type: plan.type, notes: "" }),
    });
    await fetchMaintenance();
  ...
  async function deletePlan(planId: string) {
    await fetch(`/api/printer-farm/printers/${printer.id}/maintenance-plans/${planId}`, { method: "DELETE" });
    await fetchMaintenancePlans();
    onPlansChanged();
  }

  // Merge daysUntilDue from workspace plans into per-printer plans
  const plansWithDue = maintenancePlans.map((plan) => {
    const workspacePlan = workspacePlans.find((wp) => wp.id === plan.id);
    return { ...plan, daysUntilDue: workspacePlan?.daysUntilDue ?? null };
  });
      ...
      reader.onreading = (evt) => {
        for (const record of evt.message.records) {
          if (record.recordType === "text") {
            ...
            if (found) { selectSpool(found.id); setNfcScanning(false); return; }
          }
        }
          ...
          <div style={{ padding: "16px 20px", overflowY: "auto", flex: 1 }}>
            ...
            <div className="panel panel-padded" style={{ marginBottom: "16px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "12px" }}>
                <p style={{ fontWeight: 700, fontSize: "13.5px", margin: 0 }}>{text(lang, "Wartungspläne", "Maintenance Plans")}</p>
                <button type="button" className="btn btn-ghost btn-sm" onClick={() => setShowAddPlan(true)}>
                  <IconPlus /> {text(lang, "Plan", "Plan")}
                </button>
              </div>

              {plansWithDue.length === 0 ? (
                <p style={{ fontSize: "12.5px", color: "var(--text-muted)" }}>{text(lang, "Noch keine Wartungspläne.", "No maintenance plans yet.")}</p>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                  {plansWithDue.map((plan) => {
                    const overdue = (plan.daysUntilDue ?? 0) < 0;
                    const dueSoon = !overdue && (plan.daysUntilDue ?? 999) <= plan.notifyDaysBefore;
                    const statusColor = overdue ? "var(--danger)" : dueSoon ? "#f59e0b" : "var(--text-muted)";
                    return (
                      <div key={plan.id} style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "8px", alignItems: "start", padding: "10px 12px", background: "var(--panel-muted)", borderRadius: "8px", border: overdue ? "1px solid color-mix(in srgb, var(--danger) 40%, transparent)" : "1px solid transparent" }}>
                        <div>
                          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
                            <span style={{ fontSize: "13px", fontWeight: 600 }}>{plan.label || maintenanceLabel(plan.type, lang)}</span>
                            {overdue && <span style={{ fontSize: "10px", fontWeight: 700, color: "var(--danger)", background: "color-mix(in srgb, var(--danger) 12%, transparent)", borderRadius: "20px", padding: "1px 6px" }}>⚠ {text(lang, "überfällig", "overdue")}</span>}
                            {dueSoon && <span style={{ fontSize: "10px", fontWeight: 700, color: "#f59e0b", background: "color-mix(in srgb, #f59e0b 12%, transparent)", borderRadius: "20px", padding: "1px 6px" }}>🔔 {text(lang, "bald fällig", "due soon")}</span>}
                          </div>
                          <p style={{ fontSize: "11.5px", color: "var(--text-muted)", margin: "3px 0 0" }}>
                            {text(lang, `alle ${plan.intervalDays} Tage`, `every ${plan.intervalDays} days`)}
                            {plan.lastPerformedAt ? ` · ${text(lang, "zuletzt", "last")} ${relativeDate(plan.lastPerformedAt, lang)}` : ""}
                            {plan.nextDueAt ? ` · ${text(lang, "fällig", "due")} ${relativeDate(plan.nextDueAt, lang)}` : ""}
                          </p>
                        </div>
                        <div style={{ display: "flex", gap: "4px" }}>
                          <button type="button" className="btn btn-ghost btn-sm" style={{ color: statusColor }} title={text(lang, "Als erledigt markieren", "Mark as done")} onClick={() => void markPlanDone(plan)}>✓</button>
                          <button type="button" className="btn btn-ghost btn-sm" onClick={() => setEditingPlan(plan)}><IconEdit /></button>
                          <button type="button" className="btn btn-ghost btn-sm" style={{ color: "var(--danger)" }} onClick={() => void deletePlan(plan.id)}><IconTrash /></button>
                        </div>
                      </div>
                    );
                  })}
          ...
          <div style={{ padding: "16px 20px", overflowY: "auto", flex: 1 }}>
            ...
            <div className="panel panel-padded" style={{ marginBottom: "16px" }}>
              ...
              <div style={{ marginBottom: "10px" }}>
                ...
                <select className="input" style={{ width: "100%" }} value={spoolId} onChange={(e) => selectSpool(e.target.value)}>
...
function PrinterCard({ printer, status, lang, onSelect, planSummary }: {
  printer: Printer;
  status: PrinterStatus | null;
  lang: Language;
  onSelect: () => void;
  planSummary?: { overdue: number; dueSoon: number };
}) {
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" }}>
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px", flexWrap: "wrap" }}>
            ...
            {printer.isDummy && <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 7px", borderRadius: "20px", background: "color-mix(in srgb, var(--text-muted) 15%, transparent)", color: "var(--text-muted)" }}>DUMMY</span>}
            {planSummary && planSummary.overdue > 0 && (
              <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 7px", borderRadius: "20px", background: "color-mix(in srgb, var(--danger) 15%, transparent)", color: "var(--danger)" }}>⚠ {planSummary.overdue} {text(lang, "überfällig", "overdue")}</span>
            )}
            {planSummary && planSummary.overdue === 0 && planSummary.dueSoon > 0 && (
              <span style={{ fontSize: "10px", fontWeight: 700, padding: "1px 7px", borderRadius: "20px", background: "color-mix(in srgb, #f59e0b 15%, transparent)", color: "#f59e0b" }}>🔔 {text(lang, "bald fällig", "due soon")}</span>
            )}
          </div>
...
export function PrinterFarmPageClient() {
  ...
  const fetchWorkspacePlans = useCallback(async () => {
    const res = await fetch("/api/printer-farm/maintenance-plans");
    if (res.ok) {
      const data = await res.json() as { plans: WorkspacePlanEntry[] };
      setWorkspacePlans(data.plans);
    }
  }, []);
  ...
  // planSummary: per-printer overdue/dueSoon counts for card badges
  const planSummary = new Map<string, { overdue: number; dueSoon: number }>();
  for (const p of workspacePlans) {
    const summary = planSummary.get(p.printer.id) ?? { overdue: 0, dueSoon: 0 };
    if ((p.daysUntilDue ?? 1) < 0) summary.overdue++;
    else if (p.daysUntilDue != null && p.daysUntilDue <= (p as unknown as { notifyDaysBefore?: number }).notifyDaysBefore!) summary.dueSoon++;
    planSummary.set(p.printer.id, summary);
  }
  ...
  return (
    <div>
      ...
      {(() => {
        const overduePlans = workspacePlans.filter((p) => (p.daysUntilDue ?? 0) < 0);
          ...
          <div style={{ background: "color-mix(in srgb, var(--danger) 12%, var(--panel))", border: "1px solid color-mix(in srgb, var(--danger) 40%, transparent)", borderRadius: "10px", padding: "12px 16px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <p style={{ flex: 1, fontSize: "13.5px", color: "var(--danger)", fontWeight: 600, margin: 0 }}>
              {overduePlans.length === 1
                ...
                : text(lang, `${overduePlans.length} Wartungen überfällig`, `${overduePlans.length} maintenance tasks overdue`)}
            </p>
            <button type="button" className="btn btn-ghost btn-sm" onClick={() => setDismissedOverdueBanner(true)}><IconX /></button>
          </div>
        );
      })()}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "14px" }}>
          {visiblePrinters.map((p) => (
            <div key={p.id} style={{ position: "relative" }}>
              <PrinterCard printer={p} status={statuses[p.id] ?? null} lang={lang} onSelect={() => setDetailPrinter(p)} planSummary={planSummary.get(p.id)} />
