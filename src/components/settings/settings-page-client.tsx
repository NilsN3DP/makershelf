        ...
        <div className="panel">
          <div className="panel-section">
            <Row
              ...
              desc={text(
                settings.language,
                "Optional: Verlinke deine Filamentverwaltung. Wenn aktiv, erscheint Spoolman links in der Navigation.",
                "Optional: Link your filament inventory. When enabled, Spoolman appears in the sidebar.",
              )}
            >
              <label className="toggle">
                <input
                  type="checkbox"
                  checked={settings.spoolmanEnabled}
                  onChange={(e) => updateSettings({ spoolmanEnabled: e.target.checked })}
                />
                <span className="toggle-track" />
              </label>
            </Row>
          ...
          <div className="panel-section">
            <Row title={text(settings.language, "Spoolman URL", "Spoolman URL")}>
              <input
                className="input"
                style={{ width: "min(520px, 100%)" }}
                value={settings.spoolmanUrl}
                onChange={(e) => updateSettings({ spoolmanUrl: e.target.value })}
          ...
          <div className="panel-section">
            <Row
              title={text(settings.language, "API-Key (optional)", "API key (optional)")}
              desc={text(
                settings.language,
                "Wird lokal gespeichert und später für tiefergehende Spoolman-Abfragen genutzt.",
                "Stored locally and used later for deeper Spoolman API queries.",
              )}
            >
              <input
                className="input"
                type="password"
                style={{ width: "min(520px, 100%)" }}
                value={settings.spoolmanApiKey}
                onChange={(e) => updateSettings({ spoolmanApiKey: e.target.value })}
                placeholder={text(settings.language, "Leer lassen, wenn nicht benötigt", "Leave empty if not needed")}
              />
            </Row>
          </div>
          <div className="panel-section">
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              <a
                className={`btn btn-secondary btn-sm${!settings.spoolmanUrl ? " disabled" : ""}`}
                href={settings.spoolmanUrl || "#"}
                target="_blank"
                rel="noreferrer"
                aria-disabled={!settings.spoolmanUrl}
              >
                {text(settings.language, "Spoolman öffnen", "Open Spoolman")}
              </a>
              <a className="btn btn-ghost btn-sm" href="https://github.com/Donkie/Spoolman" target="_blank" rel="noreferrer">
                GitHub ↗
              </a>
            </div>
          </div>
          ...
          <div className="panel-section">
            <Row
              title={text(settings.language, "Laser", "Laser")}
              desc={text(
                settings.language,
                "Datei-Bibliothek für SVG/AI/PDF/GCode, Browser-Vorschau, Materialprofile und Lightburn-Bridge.",
                "File library for SVG/AI/PDF/GCode, browser preview, material profiles, and Lightburn Bridge.",
              )}
            >
              <label className="toggle">
                <input
                  type="checkbox"
                  checked={settings.laserEnabled ?? false}
                  onChange={(e) => updateSettings({ laserEnabled: e.target.checked })}
                />
                <span className="toggle-track" />
              </label>
            </Row>
          ...
          {settings.laserEnabled && (
            <div className="panel-section">
              <Row
                title={text(settings.language, "Lightburn Bridge URL", "Lightburn Bridge URL")}
                desc={text(
                  settings.language,
                  "URL des lokal laufenden Lightburn Bridge Servers. Ermöglicht 'In Lightburn öffnen'.",
                  "URL of the locally running Lightburn Bridge server. Enables 'Open in Lightburn'.",
                )}
              >
                <input
                  className="input"
                  style={{ width: "min(520px, 100%)" }}
                  value={settings.lightburnBridgeUrl ?? ""}
                  onChange={(e) => updateSettings({ lightburnBridgeUrl: e.target.value })}
          ...
          <div className="panel-section">
            <Row
              title={text(settings.language, "Plotter", "Plotter")}
              desc={text(
                settings.language,
                "Datei-Bibliothek für PLT/DXF/HPGL, Schnittprofile. Separate Seite in der Navigation.",
                "File library for PLT/DXF/HPGL, cut profiles. Separate page in the navigation.",
              )}
            >
              <label className="toggle">
                <input
                  type="checkbox"
                  checked={settings.plotterEnabled ?? false}
                  onChange={(e) => updateSettings({ plotterEnabled: e.target.checked })}
                />
                <span className="toggle-track" />
              </label>
            </Row>
