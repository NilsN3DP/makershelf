...
export async function POST(request: Request) {
  ...
  const defaultTareGrams = enabled.settings.filamentDefaultTareGrams > 0 ? enabled.settings.filamentDefaultTareGrams : null;
  ...
  const spool = await prisma.filamentSpool.create({
    data: {
      ...
      tareWeightGrams: body.tareWeightGrams ?? defaultTareGrams,
